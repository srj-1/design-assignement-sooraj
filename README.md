# RTL Design & Verification Training

This repository contains daily RTL design and verification tasks completed during the training program.

## Project Structure

### Day 1
- [BCD Adder](./Day1/BCD_Adder/)
- [Ripple Carry Adder](./Day1/Ripple_Carry_Adder/)

### Day 2
- [2x4 Decoder](./Day2/2x4%20Decoder/)
- [D Flip-Flop](./Day2/DFF/)
- [SR Flip-Flop](./Day2/SR_ff/)
- [Universal Shift Register](./Day2/USR/)

### Day 3
- [Face Detector](./Day3/Face_detector/)
- [Sequence Detector](./Day3/SequenceDetector/)

### Day 4
- [Block Memory Generator](./Day4/BlockMemoryGenerator/)

### Day 5
- [BCD Adder Interface](./Day5/BCD_adder_Interface/)
- [FIFO Interface](./Day5/FIFO_interface/)

### Day 6
- [FIFO Transaction Class](./Day6/Transaction_fifo/)

### Day 8
- [APB Slave Verification](./Day8/apb/)

---

## Contents of Each Project

Each project contains:

- `design/` → RTL Design Files
- `tb/` or `Simulation/` → Testbench & Verification Files
- `readme.md` → Project Description, Schematic, and Simulation Results

---

## Tools Used

- Xilinx Vivado
- Verilog HDL
- SystemVerilog
- Git & GitHub

## Author

**Sooraj H**
ECE Engineering Student
